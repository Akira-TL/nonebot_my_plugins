...
def debug(text) -> None:
    ...
    @返回:
        none
    '''
    makedirs('src/plugins/plugins/db',exist_ok=True)
    try:
        f = open('src/plugins/plugins/db/debugconfig.json','r')
    except:
        f = open('src/plugins/plugins/db/debugconfig.json','w')
        f.write('{}')
    f.close()
    try:
        config = json.load('src/plugins/plugins/db/debugconfig.json')[__name__]
    except:
        config = 0
    if config:
...
notice_cycle = on_regex(r'(明天|后天|下周.|这周.|\d+月\d+日|每月\d+号|每周.|每天)(早上|上午|下午|晚上|\d+点\d+分|\d+点|)提醒我',priority=pro,rule=to_me())
@notice_cycle.handle()
async def notice_cycle_(event:Event):
    '''
    @说明:
        新建任务,任务规则为:"(明天|后天|下周.|这周.|\d+月\d+日|每月\d+号|每周.|每天)(早上|上午|下午|晚上|\d+点\d+分|\d+点|)提醒我"
    @返回:
        无
    '''
    
    msg = str(event.get_message())
    date = re.findall(r'明天|后天|下周.|这周.|\d+月\d+日|每月\d+号|每周.|每天',msg)[0]
    timef = re.findall(r'早上|上午|下午|晚上|\d+点\d+分|\d+点',msg)
    debug(timef)
    debug(date)
...
# 定义初始变量
    if date == '明天':
        ...
        day = date[1]
        mod = 0
    if days[tm.tm_mon-1] < day:
        day = day - days[tm.tm_mon-1]
        mon = tm.tm_mon + 1
    if mon > 12:
        mon = 1
        year = tm.tm_year + 1
# 将周期提醒的内容分开并打好模式(mod)标记,并将日期计算出来
